...
from gen_dataset import sample_batch_videos, test_batch_videos, \
                        load_videos, quick_sample_batch_videos
...
def trainIters(args, model, n_iters, print_every=1000, plot_every=100):
        ...
        if iter % print_every == 0 or iter == 1: 
            ...
            print('%s (%d %d%%) TR Loss : %.4f, Tr Score %.4f, TR Acc : %.6f, VL Loss : %.4f, VL Score %.4f, VL Acc : %.6f, LR %f' \
                                    % (timeSince(start, iter / n_iters),
                                         ...
                                         vl_acc, lr_))
            iter_end = time.time()
            if iter == 1  or iter % print_every * 100 == 0 : print ('Valid Epoch time %f' % ((iter_end-iter_start)/60.0))
...
if __name__ == '__main__':
    ...
    model = model.cuda()
    if trainF: trainIters(args, model, args.epoch, print_every=100)

    print(model)
    print('SAVE MODEL PATH %s' % args.savepath)
