...
def one_step_simulate_rnn(model, hidden, input_variable, \
    ...
    predictions = prediction.reshape([T, batch_sz, num_feat, num_bin])
...
def apply_bin_rnn(feats, state, model, hidden, params, fly=None, num_bin=51):
    ...
    if 'rnn' in params['mtype'] or 'skip' in params['mtype']:
        ...
        if torch.any(torch.isnan(feat_motion_tm1)).item():
            print("Found nans in feat_motion, replacing with 0s")
            feat_motion_tm1[torch.isnan(feat_motion_tm1)] = 0.
        vdata_t = torch.cat([feats[0], feats[1]], 0) #TODO double check the order of feats[0] and feats[1]
        fdata_t = torch.cat([vdata_t, feat_motion_tm1], 0)[:,None, None]
        fdata_t = Variable(fdata_t.permute(1,2,0))
        ...
        # this is a place-holder, returns groundtruth
        feat_motion = model[:,t,fly]
        if torch.any(torch.isnan(feat_motion)).item():
            print("Found nans in feat_motion, replacing with 0s")
            feat_motion[torch.isnan(feat_motion)] = 0.
        binidx = motion2binidx(feat_motion,params)

        n_bins = params['binedges'].shape[0]-1
        device = params['n_motions'].device
        binscores = torch.zeros(n_bins*params['n_motions'], device=device)
        for v in range(params['n_motions']):
            # select bin according to output probability
            inds=np.arange(v*n_bins,(v+1)*n_bins, device=device)
            binscores[inds[binidx[v]]] = 1
        #print "Placeholder -- using groundtruth motion"
        preds = []
