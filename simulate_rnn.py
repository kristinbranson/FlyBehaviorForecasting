...
def simulate_flies( args, real_male_flies, real_female_flies, \
    ...
    if mtype == 'data':
        ...
        ftag = str(t0)+'t0_'+str(t1)+'t1_epoch'+str(model_epoch)+'_'+sim_type+'_%s_%s_%dhid_lr%f_testvideo%d_%s' % (videotype, btype, num_hid, 0.01, testvideo_num, dtype)
    ...
    os.makedirs('./trx/%s/' % (vpath), exist_ok=True)   
    sio.savemat('./trx/%s/'%vpath+mtype+'_trx_'+ftag, simtrx)

    if bookkeepingF:
        os.makedirs('./motion/%s/' % (vpath), exist_ok=True)   
    ...
    if bookkeepingF:
        os.makedirs('./velocity/%s/' % (vpath), exist_ok=True)   
    ...
    if bookkeepingF:
        os.makedirs('./centredist/%s/' % (vpath), exist_ok=True)   
...
def parse_args():
    ...
    parser.add_argument('--mtype', type=str, default='rnn50', choices=['rnn50', 'hrnn50'])
